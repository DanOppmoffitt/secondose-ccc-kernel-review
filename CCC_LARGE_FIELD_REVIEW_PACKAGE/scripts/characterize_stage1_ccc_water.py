...
def run_field(
    ...
    *,
    ...
    mu_0_per_mm: float | None = None,
    mu_inf_per_mm: float | None = None,
    z_h_mm: float | None = None,
) -> FieldResult:
    ...
    use_depth_dependent_mu:
        Research-only TERMA hardening switch.  Default ``False`` keeps fixed
        ``mu_eff_per_mm`` behavior unchanged.
    mu_0_per_mm, mu_inf_per_mm, z_h_mm:
        Depth-dependent TERMA attenuation parameters used only when
        ``use_depth_dependent_mu`` is True.
    """
    fs_label = f"{field_size_cm:g}x{field_size_cm:g}"
    ...
    if kernel_convention is None:
        if use_depth_dependent_mu:
            result = compute_stage1(geometry, beam, calibration, kernel, **terma_kwargs)
        else:
