...
def depth_dependent_mu_eff(
    depth_mm: np.ndarray | float,
    *,
    mu_0_per_mm: float,
    mu_inf_per_mm: float,
    z_h_mm: float,
) -> np.ndarray:
    """Return the experimental hardened effective attenuation coefficient.

    The opt-in beam-hardening model is::

        mu_eff(z) = mu_inf + (mu_0 - mu_inf) * exp(-z / z_h)

    with all lengths in mm and coefficients in 1/mm.  This helper only returns
    the local coefficient; attenuation must be computed from the cumulative
    integral by :func:`cumulative_hardened_attenuation`, not by
    ``exp(-mu_eff(z) * z)``.
    """
    mu0 = float(mu_0_per_mm)
    mui = float(mu_inf_per_mm)
    zh = float(z_h_mm)
    if not (np.isfinite(mu0) and np.isfinite(mui) and np.isfinite(zh)):
        raise ValueError("mu_0_per_mm, mu_inf_per_mm, and z_h_mm must be finite")
    if mu0 < 0.0 or mui < 0.0:
        raise ValueError("mu_0_per_mm and mu_inf_per_mm must be non-negative")
    if zh <= 0.0:
        raise ValueError(f"z_h_mm must be > 0, got {z_h_mm}")

    z = np.maximum(np.asarray(depth_mm, dtype=np.float64), 0.0)
    return mui + (mu0 - mui) * np.exp(-z / zh)


def cumulative_hardened_attenuation(
    depth_mm: np.ndarray | float,
    *,
    mu_0_per_mm: float,
    mu_inf_per_mm: float,
    ...
    """Return ``exp(-integral_0^z mu_eff(s) ds)`` for the hardening model.

    For ``mu_eff(s) = mu_inf + (mu_0 - mu_inf) exp(-s / z_h)``, the analytic
    cumulative integral is::

        integral_0^z mu_eff(s) ds
          = mu_inf * z + (mu_0 - mu_inf) * z_h * (1 - exp(-z / z_h))

    Therefore the primary attenuation is::

        attenuation(z) = exp(-mu_inf*z
                             - (mu_0 - mu_inf)*z_h*(1 - exp(-z/z_h)))

    This is intentionally separate from :func:`depth_dependent_mu_eff` to avoid
    the incorrect shortcut ``exp(-mu_eff(z) * z)``.
    """
    # Reuse validation from the local-coefficient helper.
    _ = depth_dependent_mu_eff(
        0.0,
    ...
    zh = float(z_h_mm)
    z = np.maximum(np.asarray(depth_mm, dtype=np.float64), 0.0)
    cumulative = mui * z + (mu0 - mui) * zh * (1.0 - np.exp(-z / zh))
...
def compute_terma_water(
    ...
    *,
    ...
    use_depth_dependent_mu: bool = False,
    mu_0_per_mm: Optional[float] = None,
    mu_inf_per_mm: Optional[float] = None,
    z_h_mm: Optional[float] = None,
) -> TermaVolume:
    ...
    Default fixed-mu mode preserves the historical expression::

        TERMA(p) = (SAD / d_src)^2 * exp(-mu * depth) * aperture(p) * mu_eff

    When ``use_depth_dependent_mu=True`` (research only), the primary
    attenuation is instead computed from the cumulative integral of
    ``mu_eff(z)``::

        mu_eff(z) = mu_inf + (mu_0 - mu_inf) * exp(-z / z_h)
        attenuation(z) = exp(-integral_0^z mu_eff(s) ds)

    and the local ``mu_eff(z)`` is used for the TERMA interaction factor.
    ...
    mu_eff_per_mm:
        Effective linear attenuation coefficient for 6 MV in water (1/mm).
    use_depth_dependent_mu:
        Research-only opt-in switch.  ``False`` keeps fixed ``mu_eff_per_mm``
        behavior unchanged.
    mu_0_per_mm, mu_inf_per_mm, z_h_mm:
        Depth-dependent attenuation parameters.  Required when
        ``use_depth_dependent_mu`` is True.  Units: 1/mm, 1/mm, and mm.
    ...
    # Primary attenuation in water.  The default branch intentionally keeps the
    # historical fixed-mu expression so callers with use_depth_dependent_mu=False
    # reproduce existing results bit-for-bit.
    if use_depth_dependent_mu:
        if mu_0_per_mm is None or mu_inf_per_mm is None or z_h_mm is None:
            raise ValueError(
                "mu_0_per_mm, mu_inf_per_mm, and z_h_mm are required when "
                "use_depth_dependent_mu=True"
            )
        atten = cumulative_hardened_attenuation(
            depth_mm,
            mu_0_per_mm=float(mu_0_per_mm),
            mu_inf_per_mm=float(mu_inf_per_mm),
            z_h_mm=float(z_h_mm),
        )
        terma_mu = depth_dependent_mu_eff(
            depth_mm,
            mu_0_per_mm=float(mu_0_per_mm),
            mu_inf_per_mm=float(mu_inf_per_mm),
            z_h_mm=float(z_h_mm),
    ...
    terma_rel = inv_sq * atten * aperture * terma_mu
...
def compute_stage1(
    ...
    *,
    ...
    use_depth_dependent_mu:
        Research-only opt-in TERMA hardening switch.  Default ``False`` keeps
        fixed-mu behavior unchanged.
    mu_0_per_mm, mu_inf_per_mm, z_h_mm:
        Optional hardening parameters passed only to TERMA generation.
    ref_depth_mm:
    ...
    # 1. TERMA
    terma_vol = compute_terma_water(
        geometry,
        beam,
        mu_eff_per_mm=mu_eff_per_mm,
        use_depth_dependent_mu=use_depth_dependent_mu,
        mu_0_per_mm=mu_0_per_mm,
        mu_inf_per_mm=mu_inf_per_mm,
        z_h_mm=z_h_mm,
    )
