...
from DoseCalc.dose_engine.ccc_transport import (
    ...
    _SAD_MM,
    _voxel_world_coords,
    cumulative_hardened_attenuation,
    depth_dependent_mu_eff,
...
def compute_terma_stage5(
    ...
    *,
    ...
    When ``use_depth_dependent_mu=True`` (research only), WEPL is used as the
    depth coordinate for the hardened cumulative attenuation integral.
    ...
    use_depth_dependent_mu : bool
        Research-only TERMA hardening switch. Default False keeps fixed-mu
        behavior unchanged.
    mu_0_per_mm, mu_inf_per_mm, z_h_mm : float, optional
        Hardening parameters used only when ``use_depth_dependent_mu`` is True.
